...
export default {
  ...
  methods: {
            ...
            (enter) =>
              enter
                ...
                .attr(
                  "transform",
                  (d) =>
                    `translate(${x((this.prev.get(d) || d).value)},${y((this.prev.get(d) || d).rank)})`,
                )
            ...
            (exit) =>
              exit
                ...
                .attr(
                  "transform",
                  (d) =>
                    `translate(${x((this.next.get(d) || d).value)},${y((this.next.get(d) || d).rank)})`,
                )
                .call((g) =>
                  g
                    .select("tspan")
                    .tween("text", (d) =>
                      this.textTween(d.value, (this.next.get(d) || d).value),
                    ),
                ),
          )
          .call((bar) =>
            bar
              ...
              .call((g) =>
                g
                  .select("tspan")
                  .tween("text", (d) =>
                    this.textTween((this.prev.get(d) || d).value, d.value),
                  ),
              ),
          ));
    },
    ...
    initialize(newValue) {
      ...
      this.keyframes.push([
        new Date(kb),
        this.rank((name) => b.get(name) || 0),
      ]);
      this.color = this.colorf(thedata);
      this.nameframes = d3.groups(
        this.keyframes.flatMap(([, data]) => data),
        (d) => d.name,
      );
      this.prev = new Map(
        this.nameframes.flatMap(([, data]) => d3.pairs(data, (a, b) => [b, a])),
      );
      this.next = new Map(
        this.nameframes.flatMap(([, data]) => d3.pairs(data)),
      );
